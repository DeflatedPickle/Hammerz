...
public class HammerInfoHandler {
	...
	public static float getStrengthVsBlock(ItemStack stack, IBlockState state) {
		return state.getMaterial() != Material.IRON && state.getMaterial() != Material.ANVIL && state.getMaterial() != Material.ROCK ? 1.0F : (getEfficiency(stack) * ConfigOptions.EfficiencyMultiplier);
	}
	...
	public static boolean getUsesLP(ItemStack stack){
		if(getHammerType(stack) == null){
			return false;
		}
		return  getHammerType(stack).getUsesLP();
	}

	public static boolean getUsesDW(ItemStack stack){
    	if(getHammerType(stack) == null){
    		return false;
		}
		return getHammerType(stack).getUsesDW();
	}
	...
	public static HammerType getHammerType(ItemStack stack) {
		if (stack.getItem() != null && stack.getItem() == HammerX_Items.ItemHammer) {
	...
	public static HammerType getHammerType(String hammerTypeName) {
		if (hammerTypeName != null) {
			...
			List<HammerType> typeList = HammerX_Items.hammerTypes;
			for (HammerType hammerType : typeList) {
				if (hammerType.getName().toLowerCase().contentEquals(hammerTypeName.toLowerCase())) {
					return hammerType;
				}
			}
			/*
